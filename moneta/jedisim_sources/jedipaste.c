 ...
 *
 *
 * Compile     : gcc -Wall -O3 -o jedipaste jedipaste.c -lm -lcfitsio
 * Run         : ./jedipaste 12288 12288 out1/trial0_distortedlist.txt out1/trial0_HST.fits
 ...
 *       into a single large embedded image: out1/trial0_HST.fits according
 *       to input textfile out1/trial0_distortedlist.txt (created by jedicatalog)
 * 
 ...
 * 
 *
 * Info: This program also combines 6 convolved files from
 *       out1/trial0_convolvedlist.txt (created by jedicatalog)
 *       takes in the fitsfiles
 *       out1/convolved/convolved_band_0_to_5.fits
 ...
 *
 *       It reads the left corners of images (nx,ny) from the config file: config.conf
 */
#include <stdio.h>
#include <math.h>
#include <string.h>
...
int main(int argc, char *argv[]){
    ...
    fits_create_file(&ffptr, buf, &status);
