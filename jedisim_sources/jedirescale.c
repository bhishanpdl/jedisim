 ...
 *
 * Compile     : gcc -Wall -O3 -o jedirescale jedirescale.c  -lm -lcfitsio
 *
 * Usage       : jedirescale input_file pixscale new_pixscale trimx trimy output_file
 * Run         : ./jedirescale out1/trial0_HST_convolved.fits 0.03 0.2 480 480 out1/rescaled_convolved_lsst_0.fits
 *
 *
 ...
 *  # scale the image down from HST to LSST scale and trim the edgescolor
    run_process("jedirescale", ['./executables/jedirescale',
      config['HST_convolved_image'],  # out1/trial0_HST_convolved.fits
      config['pix_scale'],            # 0.06
      config['final_pix_scale'],      # 0.2
      config['x_trim'],               # 480
      config['y_trim'],               # 480
      outfile[i]])                    # from: physics_settings/rescaled_out0.txt
 *                                    # jedisim_out/rescaled_out0/rescaled_0.fits
 *
...
int main(int argc, char *argv[]){
    ...
    for(band = 0; band < NUMBANDS; band++){
        ...
        if (image == NULL){
            fprintf(stderr, "Error: cannot allocate memory for band %li.\n", band);
            exit(1);
        }
        //fprintf(stdout,"bpixel = {%li, %li}\n", bpixel[0], bpixel[1]);

        //read in the image
        fprintf(stdout, "Reading in %s band %li, size: %.0fMB. This may take a few minutes for large images.\n", infile, band, (float) inaxes[0]*inaxes[1]*sizeof(float)/(1000000*NUMBANDS));
